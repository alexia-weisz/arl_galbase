...
_FILE_DIR = '/n/home00/lewis.1590/research/galbase_allsky/ngc0628_ims'
_INTFILE_DIR = os.path.join(_FILE_DIR, 'int')
_BGFILE_DIR = os.path.join(_FILE_DIR, 'skybg')
_RRHRFILE_DIR = os.path.join(_FILE_DIR, 'rrhr')
_INTSUBSKYFILE_DIR = os.path.join(_FILE_DIR, 'bgsub')
...
def galex(band='fuv', ra_ctr=None, dec_ctr=None, size_deg=None, index=None, name=None):
    ...
    galaxy_mosaic_file = os.path.join(_HOME_DIR, '_'.join([name, band]).upper() + '.FITS')
    ...
    if not os.path.exists(galaxy_mosaic_file):
        ...
        try:
            ...
            montage.mosaic(input_dir, final_dir, header=hdr_file, background_match=True, combine='mean')
